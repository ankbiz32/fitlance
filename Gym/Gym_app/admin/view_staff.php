		...
		$sno    = 1;
			if (mysqli_affected_rows($con) != 0) {
				while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
					...
					echo "<td width='250'><form action='?vis=staffpay' method='post'><input type='hidden' name='name' value='" . $stafid . "'/><input type='submit' value='Add Payment' class='btn btn-sm btn-info pull-left'/></form><form action='?vis=view_staffpay' method='post'><input type='hidden' name='name' value='" . $stafid . "'/><input type='submit' value='View Staff Payment' class='btn btn-sm btn-success pull-left'/></form></td></tr>";
					$sno++;
					$stafid = 0;
				}
			}
