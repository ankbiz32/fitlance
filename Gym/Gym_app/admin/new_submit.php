...
if (isset($_POST['p_name']) && isset($_POST['mem_type']) && isset($_POST['total']) && isset($_POST['paid'])) {
    ...
    if (mysqli_num_rows($result2) == 0){
        ...
        mysqli_query($con,"INSERT INTO user_data (wait,newid,name,address,zipcode,birthdate,contact,email,curr_date,landline,joining,workout_time_id,sex,activity,is_combo_member,branch_id,insert_by)VALUES('$wait','$p_id','$full_name','$address','$zipcode','$birthdate','$contact','$email','$curr_date','$landline','$joindate','$workout_time','$sex','$activity','$combo','$branch_id',$insert_by)");
		//print_r($_POST);
		$total = $total - $dis;
		$expiry = $expiry ;
		$expiry1 = date('d-m-Y',strtotime( $expiry ));
			
	  mysqli_query($con, "INSERT INTO subsciption (mem_id,bank_id,name,sub_type,paid_date,total,paid,dis,total_dis,expiry,invoice,sub_type_name,bal,exp_time,payment_method,cheque_no,renewal,curr_date,pay_date,is_combo_member,branch_id,insert_by)
	VALUES ('$p_id','$bank_id','$full_name','$mem_type','$joindate','$total','$paid','$dis','$total_dis','$expiry','$invoice','$name_type','$bal','$exp_time','$paymentdata','$chequeno','yes','$curr_date','$curr_date','$combo','$branch_id','$insert_by')");
	...
	VALUES ('$p_id','$bank_id','$full_name','$mem_type','$total','$paid','$dis','$total_dis','$expiry','$invoice','$name_type','$bal','$paymentdata','$chequeno','yes','$curr_date','$insert_by')");
		...
		if (!empty($_POST['trainer_name']) && !empty($_POST['trainer_type_id']) && !empty($_POST['totalt']) && !empty($_POST['paidt'])) {
			...
			mysqli_query($con, "INSERT INTO trainer_pay (member_id,member_name,staff_id,staff_name,trainer_type_id,bank_id,paid_date,join_date,payment_method,expiry_date,cheque_no,total,paid,invoice,paybalance,expiry,renewal,exp_time,branch_id,insert_by)
		        VALUES('$p_id','$full_name','$trainer_name','$name_staff','$trainer_type_id','$bank_id_tr','$date','$date','$payment_method_tr','$expiry_tr','$cheque_no_tr','$total_tr','$paid_tr','$invoice_tr','$paybalance','$expiry_tr','yes','$exp_time_tr','$branch_id','$insert_by')");
		}
