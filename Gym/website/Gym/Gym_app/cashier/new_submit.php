...
if (isset($_POST['p_name']) && isset($_POST['mem_type']) && isset($_POST['total']) && isset($_POST['age']) && isset($_POST['paid'])) {
    ...
    $image = $_FILES['custimage']['tmp_name'];
    $imageName = $_FILES['custimage']['name'];
	$path = "../images/";
	move_uploaded_file($image, $path.$imageName);
    $query = "select * from user_data";
    //echo $query;
    $result = mysqli_query($con, $query);
	$p_id = mysqli_num_rows($result) + 1;
	mysqli_query($con, "INSERT INTO user_data(wait,newid,name,address,zipcode,birthdate,contact,email,pic_add,height,weight,nationality,facebookaccount,twitteraccount,contactperson,previousgym,yearstraining,joining,age,sex,proof) VALUES ('no',$p_id,'$full_name','$address','$zipcode','$birthdate', '$contact','$email','$imageName','$height','$weight','$nationality','$facebookaccount','$twitteraccount','$contactperson','$previousgym', '$yearstraining','$date','$age','$sex','$proof')");
            $bal = $total - $paid;
...
?>";
	...
	<body>
		<header>
			...
			<address>
				<p>Cluebix Application Gym</p>
				<p>158,IT Park Road,Loksewa Nagar,Nagpur</p>
				<p>Landline:+0712-2295706</p><p>www.cluebix.com (info.cluebix@gmail.com)</p><br><p><div id="barcodeTarget" class="barcodeTarget"></div>
...
?></span></td>
		...
		<aside>
			...
			<div  >
				<p>1). All members must abide our TnC / rules otherwise membership may be withdrawn. </br></br>2). The payment is not transferable and non-refundable.</br></br>3).Fee Should be submitted within 5 business days before subjected to expiration, otherwise Rs.10 /Day will be charged.</br></br>4).All users should dress appropriately Or as per advised.</br></br>5).Smoking is NOT permitted at the Gym site.
			</div>
		</aside><center><br><br><a href="index.php?vis=view_mem">Cluebix Application Gym( www.cluebix.com )</center>
	</body>
</html>